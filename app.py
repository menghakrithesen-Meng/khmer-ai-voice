...
# 🎨 CUSTOM CSS (PRESET BUTTON COLORS FIX)
st.markdown("""
<style>
...
with tab2:
    st.info("Custom Voice per Line. Audio synced to SRT Start Time.")
    ...
    if srt_file:
        if "srt_lines" not in st.session_state or st.session_state.get("last_srt") != srt_file.name:
            ...
            # Load Default Global Settings for ALL lines
        ...
        with st.container(height=600):
            for idx, sub in enumerate(st.session_state.srt_lines):
                ...
                # Check if values changed manually (Disable Preset Color)
                ...
                with c_presets:
                    ...
                    for slot_id in range(1, 7):
                        ...
                        # COLOR LOGIC: Red if Active, Gray if Not
        ...
        if st.button("🚀 Generate Full Audio (Synced)", type="primary"):
            ...
            last_end = st.session_state.srt_lines[-1]['start'] + 5000 # Estimating end time buffer
            ...
            try:
                ...
                for i, sub in enumerate(st.session_state.srt_lines):
                    ...
                    # GET SPECIFIC SETTINGS FOR THIS LINE
                    s = st.session_state.line_settings[i]
                    
                    # Generate with SPECIFIC voice/rate/pitch
