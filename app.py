...
REMEMBER_FILE = "remember_key.json"   # ⭐ Remember last key (server-side)
...
# ==========================================
# 3. AUTH FLOW (with server-side remember)
# ==========================================
...
# 1) AUTO-LOGIN FROM REMEMBER_FILE
if not st.session_state.auth:
    rem_key = load_remembered_key()
    if rem_key:
        s, d = check_access_key(rem_key)
        if s == "Valid":
            st.session_state.auth = True
            st.session_state.ukey = rem_key
            st.session_state.days = d

# 2) LOGIN FORM
if not st.session_state.auth:
    default_val = load_remembered_key() or ""
    key = st.text_input("🔑 Access Key", type="password", value=default_val)
    if st.button("Login"):
    ...
    st.info(
        ...
        "Example: `http://localhost:8501/?view=admin`"
    )
    st.stop()
