...
<style>
    ...
    /* Active Button Style (Red when selected) */
...
# --- SRT PARSER (Time Sync Logic) ---
def srt_time_to_ms(time_str):
    try:
        # Parse: 00:00:01,500 --> 00:00:04,000
...
if st.query_params.get("view") == "admin":
    ...
    if not st.session_state.admin_auth:
        ...
        st.markdown("---")
        if st.button("⬅️ Back to App"): st.query_params.clear(); st.rerun()
        st.stop()

    with st.sidebar:
        if st.button("Logout Admin"): st.session_state.admin_auth = False; st.rerun()
        if st.button("Go to User App"): st.query_params.clear(); st.rerun()
...
if not st.session_state.auth:
    ...
    st.markdown("---")
    if st.button("🔐 Admin Login"): st.query_params["view"] = "admin"; st.rerun()
    st.stop()
...
with tab2:
    st.info("SRT Mode: Audio will sync exactly to SRT timestamps.")
    ...
    if srt_file:
        ...
        if "srt_lines" not in st.session_state or st.session_state.get("last_srt") != srt_file.name:
            ...
            # Default: Use Global Settings for all lines
            for _ in st.session_state.srt_lines:
                st.session_state.line_settings.append({
                    ...
                    "active_slot": None # Track which preset is active
                })
        ...
        with st.container(height=600):
            for idx, sub in enumerate(st.session_state.srt_lines):
                ...
                # Get current config for this line
                ...
                # Detect Manual Change -> If changed, remove 'active' color from preset
                active_slot = cur['active_slot']
                ...
                with c_presets:
                    ...
                    for slot_id in range(1, 7):
                        ...
                        # Button Label: Name or Number
                        label = pd['name'] if pd and pd.get('name') else str(slot_id)
                        if len(label) > 4: label = label[:4]
                        
                        # Highlight Logic: If this slot is active, use Primary Color
        ...
        # 3. GENERATE (SYNC LOGIC)
        if st.button("🚀 Generate Full Audio (Strict Sync)", type="primary"):
            progress = st.progress(0)
            status = st.empty()
            
            # Determine Total Duration based on last subtitle
            last_sub_end = st.session_state.srt_lines[-1].get('end', 0) if st.session_state.srt_lines else 0
            ...
            try:
                ...
                for i, sub in enumerate(st.session_state.srt_lines):
                    ...
                    # --- OVERLAY LOGIC ---
                    # Paste the clip EXACTLY at the SRT start time
