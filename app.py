...
with tab2:
    st.info("Click a preset button (1-6) to apply voice settings to that line.")
    ...
    if srt_file:
        ...
        with st.container(height=600):
            for idx, sub in enumerate(st.session_state.srt_lines):
                ...
                # Get current settings for this line
                cur = st.session_state.line_settings[idx]
                
                # --- CONTROLS (Widgets linked to state) ---
                # Important: key=f"..." makes it independent
                ...
                # Detect Manual Changes -> Reset active slot
                ...
                st.session_state.line_settings[idx] = {
                    "voice": new_v, "rate": new_r, "pitch": new_p, "active_slot": active_slot
                }
                    ...
                    for slot_id in range(1, 7):
                        ...
                        if cols[slot_id-1].button(label, key=f"btn_{idx}_{slot_id}", type=b_type, help=f"Apply {label}"):
                            if pd:
                                ...
                                # FORCE REFRESH (So widgets update)
                                st.rerun()
