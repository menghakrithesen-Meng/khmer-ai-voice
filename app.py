...
with tab2:
    st.info("SRT Mode: Customize per line. Audio is synced to SRT start time.")
    ...
    if srt_file:
        ...
        with st.container(height=600):
            for idx, sub in enumerate(st.session_state.srt_lines):
                ...
                # Logic: If user changes values manually -> Remove 'active' color from preset
        ...
        if st.button("🚀 Generate Full Audio (Synced)", type="primary"):
            ...
            try:
                ...
                for i, sub in enumerate(st.session_state.srt_lines):
                    ...
                    # 2. SYNC TIME (Calculation)
                    target_start = sub['start']
                    
                    # If next line starts LATER than current audio end -> Add Silence
                    if target_start > current_timeline_ms:
                        silence_gap = target_start - current_timeline_ms
                        final_mix += AudioSegment.silent(duration=silence_gap)
                        current_timeline_ms += silence_gap
                    
                    # Append Audio
                    final_mix += clip
                    current_timeline_ms += len(clip)
                    
                    try: os.remove(raw_path)
                    except: pass
                    progress.progress((i+1)/len(st.session_state.srt_lines))
                
                status.success("Done! Audio is synced to SRT timestamps.")
                buf = io.BytesIO()
