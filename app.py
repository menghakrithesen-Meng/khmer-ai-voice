...
# ==========================================
# 1. FUNCTIONS (TKINTER LOGIC ADAPTED)
# ==========================================
def load_json(path):
...
# --- SRT PARSER (ROBUST TIME SYNC) ---
def srt_time_to_ms(time_str):
    # Converts '00:00:05,000' to milliseconds
    try:
...
def parse_srt(content):
    ...
    for b in blocks:
        ...
        if len(lines) >= 3:
            time_idx = 1
            # Robustly find the timestamp line
            if '-->' not in lines[1]:
...
with st.sidebar:
    ...
    for i in range(1, 7):
        ...
        if c1.button(f"📂 {bn}", key=f"load_{i}", use_container_width=True):
            if sp:
        ...
        if c2.button("💾", key=f"save_{i}"):
            data = {"voice": st.session_state.g_voice, "rate": st.session_state.g_rate, "pitch": st.session_state.g_pitch}
        ...
        if txt:
            with st.spinner("Generating..."):
                try:
                    ...
                    st.audio(buf)
                    st.download_button("Download", buf, "audio.mp3")
                except Exception as e:
                    st.error(f"Error: {e}")

with tab2:
    st.info("SRT Mode: Audio will match SRT Start Times perfectly (Tkinter Logic).")
    ...
    if srt_file:
        ...
        if st.button("🚀 Generate Full Audio (Strict Sync)", type="primary"):
            ...
            # --- LOGIC: TKINTER STYLE OVERLAY ---
            # 1. Determine max duration needed
            last_end = st.session_state.srt_lines[-1]['end'] if st.session_state.srt_lines else 0
            # Create a silent base track covering the whole duration + buffer
            final_mix = AudioSegment.silent(duration=last_end + 3000)
            ...
            try:
                ...
                for i, sub in enumerate(st.session_state.srt_lines):
                    ...
                    # Overlay exactly at start time (This guarantees sync)
                ...
                status.success("Done! Audio matches SRT Timing.")
                buf = io.BytesIO()
