...
<style>
    ...
    /* Active Button Color (Primary = Red/Pink) */
...
with tab2:
    st.info("Custom Voice/Speed/Pitch per line. Audio synced to SRT Time.")
    ...
    if srt_file:
        if "srt_lines" not in st.session_state or st.session_state.get("last_srt") != srt_file.name:
            ...
            # Default: Use Global Settings for ALL
        ...
        with st.container(height=600):
            for idx, sub in enumerate(st.session_state.srt_lines):
                ...
                # Detect Manual Change -> Reset Active Slot color
                ...
                with c_presets:
                    ...
                    for slot_id in range(1, 7):
                        ...
                        # COLOR LOGIC: Red (Primary) if active, else Secondary
        ...
        if st.button("🚀 Generate Full Audio (Strict Sync)", type="primary"):
            ...
            final_mix = AudioSegment.silent(duration=last_start + 5000)
