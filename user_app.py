...
<style>
    ...
    .srt-box {
        ...
        padding: 8px; margin-bottom: 2px; border-left: 3px solid #8b5cf6; font-size: 14px;
    }
    .srt-row-container { padding-bottom: 10px; border-bottom: 1px solid #334155; margin-bottom: 10px; }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 1. SHARED DATA FUNCTIONS
# ==========================================
def load_json(path):
...
async def process_srt_with_progress(subs, line_configs, v_opts, pad_ms, progress_bar, status_text):
    ...
    for i, sub in enumerate(subs):
        ...
        if voice_selection not in v_opts: vc_code = list(v_opts.values())[0]
        else: vc_code = v_opts[voice_selection]
...
if view_mode == "admin":
    ...
    with tab1:
        st.subheader("បង្កើត Key ថ្មី")
        col1, col2 = st.columns([2, 1])
        days = col1.number_input("ចំនួនថ្ងៃ (Duration)", min_value=1, value=30)
        if col2.button("Generate Now", use_container_width=True):
            k = admin_generate_key(days)
            st.success(f"Key Created Successfully!")
            st.code(k, language="text")

    with tab2:
        st.subheader("បញ្ជី Keys ទាំងអស់")
        data = load_json(KEYS_FILE)
        ...
        if not data:
            ...
            sorted_keys = sorted(data.items(), key=lambda x: x[1].get('activated_date', '') or '', reverse=True)
            for key, info in sorted_keys:
                if search and search.upper() not in key: continue
                
                with st.expander(f"🔑 {key} ({info['status']}) - {info['duration_days']} Days"):
                    c1, c2, c3 = st.columns(3)
                    c1.write(f"**Status:** {info['status']}")
                    c2.write(f"**Active Date:** {info['activated_date']}")
                    is_locked = "🔒 Yes" if info.get('bound_device') else "🔓 No"
                    c3.write(f"**Device Locked:** {is_locked}")
                    
                    b1, b2 = st.columns(2)
                    if b1.button("Reset Device Lock", key=f"rst_{key}"):
                        if admin_reset_device(key):
                            st.success(f"Unlocked device for {key}")
                            time.sleep(1)
                            st.rerun()
                    
                    if b2.button("DELETE KEY", key=f"del_{key}", type="primary"):
                        if admin_delete_key(key):
                            st.error(f"Deleted {key}")
                            time.sleep(1)
                            st.rerun()
            ...
            with st.container(height=600):
                for i, sub in enumerate(subs):
                    ...
                    with c1: 
                        ...
                        sel_vc = st.selectbox("Voice", list(v_opts.keys()), key=k_v, index=list(v_opts.keys()).index(curr_v), label_visibility="collapsed")
